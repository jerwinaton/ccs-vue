...
const closeModalBtns = document.querySelectorAll('[data-dismiss="modal"], .close-modal');
...
async function handleScheduleSubmit() {
    ...
    try {
        ...
        const applicantDoc = await getDoc(doc(db, 'applicants', applicantSelect.value));
        const interviewerDoc = await getDoc(doc(db, 'users', interviewerSelect.value));
        
        const applicantData = applicantDoc.data();
        const interviewerData = interviewerDoc.data();

        // Create interview document
        const interviewDateTime = new Date(`${interviewDate.value}T${interviewTime.value}`);
...
function setupEventListeners() {
    ...
    const dismissModalButtons = document.querySelectorAll('[data-dismiss="modal"]');
    ...
    dismissModalButtons.forEach(btn => btn.addEventListener('click', closeModals));
...
async function updateStatistics() {
    try {
        ...
        const todayInterviews = currentInterviews.filter(interview => 
            new Date(interview.dateTime).setHours(0, 0, 0, 0) === today.getTime()
        ).length;

        const pending = currentInterviews.filter(interview => 
            interview.status === 'scheduled'
        ).length;

        const completed = currentInterviews.filter(interview => 
            interview.status === 'completed'
        ).length;

        const passed = currentInterviews.filter(interview => 
            interview.status === 'completed' && interview.score >= 3
        ).length;

        todayCount.textContent = todayInterviews;
        pendingCount.textContent = pending;
        completedCount.textContent = completed;
        passRate.textContent = completed ? `${Math.round((passed / completed) * 100)}%` : '0%';
    } catch (error) {
...
function renderInterviews() {
    ...
    interviewsTableBody.innerHTML = pageInterviews.map(interview => `
        <tr>
            ...
            <td>
                <button class="btn-icon view-details-btn" onclick="viewDetails('${interview.id}')">
                    <i class="fas fa-eye"></i>
                </button>
                <button class="btn-icon" onclick="editInterview('${interview.id}')">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="btn-icon" onclick="deleteInterview('${interview.id}')">
