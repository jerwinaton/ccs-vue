...
function updateExamCards() {
  ...
  examCards.forEach((card) => {
    ...
    if (exam) {
      ...
      if (info) {
        info.innerHTML = `
                    <p><i class="fas fa-clock"></i> Duration: ${
                      exam.duration
                    }</p>
                    <p><i class="fas fa-${
                      exam.questions ? "list" : "code"
                    }"></i>
                        ${
                          exam.questions
...
toggleButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    button.querySelector("i").className = `fas fa-${
      type === "password" ? "eye-slash" : "eye"
    }`;
  });
});
  ...
  if (document.querySelector(".exam-section")) {
    ...
    document.addEventListener("visibilitychange", () => {
      if (examState.examStarted && document.hidden) {
        ...
        if (examState.warningCount <= examState.maxWarnings) {
          showNotification(
            `Warning ${examState.warningCount}/${examState.maxWarnings}: Switching tabs/windows is not allowed!`,
            "warning"
          );
        } else {
          showNotification(
            "Maximum warnings reached. Your exam will be submitted.",
            "error"
          );
          submitExam();
        }
    ...
    document
      ...
      ?.addEventListener("click", async () => {
        try {
          ...
          showNotification(
            "Please allow fullscreen to start the exam",
            "error"
          );
        }
      });
    ...
    document.addEventListener("fullscreenchange", () => {
      if (examState.examStarted && !document.fullscreenElement) {
        ...
        if (examState.warningCount <= examState.maxWarnings) {
          showNotification(
            `Warning ${examState.warningCount}/${examState.maxWarnings}: Fullscreen is required!`,
            "warning"
          );
