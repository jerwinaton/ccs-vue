...
function updateStatistics() {
    ...
    const passedCount = interviews.filter(interview => 
...
function renderInterviews() {
    ...
    paginatedInterviews.forEach(interview => {
        ...
        row.innerHTML = `
            <td><input type="checkbox" class="interview-checkbox" value="${interview.id}"></td>
            ...
            <td>
                <button class="btn-view" onclick="viewInterview('${interview.id}')">
...
function filterInterviews() {
    ...
    return interviews.filter(interview => {
        const matchesSearch = interview.applicantName.toLowerCase().includes(searchTerm) ||
...
async function scheduleInterview() {
    ...
    try {
        const interviewData = {
            applicantName: document.getElementById('applicantSelect').options[
                document.getElementById('applicantSelect').selectedIndex
            ].text,
            applicantId: formData.get('applicantSelect'),
            date: new Date(formData.get('interviewDate') + 'T' + formData.get('interviewTime')).toISOString(),
            duration: parseInt(formData.get('duration')),
            interviewer: document.getElementById('interviewerSelect').options[
...
async function viewInterview(interviewId) {
    ...
    document.getElementById('detailApplicantName').textContent = interview.applicantName;
    document.getElementById('detailApplicantEmail').textContent = interview.applicantEmail || '-';
    document.getElementById('detailApplicantPhone').textContent = interview.applicantPhone || '-';
    document.getElementById('detailDateTime').textContent = formatDateTime(interview.date);
    document.getElementById('detailDuration').textContent = `${interview.duration} minutes`;
...
async function saveEvaluation() {
    const interviewId = document.querySelector('[data-interview-id]').dataset.interviewId;
    const interview = interviews.find(i => i.id === interviewId);
    
    try {
        const evaluationData = {
            technicalSkills: {
                problemSolving: parseInt(document.querySelector('input[name="problemSolving"]:checked').value),
                technicalKnowledge: parseInt(document.querySelector('input[name="technicalKnowledge"]:checked').value)
            },
            softSkills: {
                communication: parseInt(document.querySelector('input[name="communication"]:checked').value),
                attitude: parseInt(document.querySelector('input[name="attitude"]:checked').value)
            },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('selectAll').addEventListener('change', (e) => {
        document.querySelectorAll('.interview-checkbox').forEach(checkbox => 
