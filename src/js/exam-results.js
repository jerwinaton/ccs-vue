...
function updateResultsDisplay(results) {
  ...
  Object.entries(results.sectionScores).forEach(([section, score]) => {
    const sectionCard = document.querySelector(
      `.section-card[data-section="${section}"]`
    );
    if (sectionCard) {
      sectionCard.querySelector(
        ".section-score"
      ).textContent = `${score.obtained}/${score.total}`;
      sectionCard.querySelector(".progress").style.width = `${
        (score.obtained / score.total) * 100
      }%`;
...
function updateFeedback(feedback) {
  ...
  const improvementsList = document.querySelector(
    ".feedback-item:last-child ul"
  );
...
document.querySelector(".schedule-btn").addEventListener("click", async () => {
  try {
    ...
    if (slotsDoc.exists()) {
      ...
      alert(
        "No interview slots available at the moment. Please try again later."
      );
    }
  } catch (error) {
