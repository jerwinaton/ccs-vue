...
class AdminDashboard {
    ...
    async loadStatistics() {
        try {
            ...
            document.getElementById('totalApplicants').textContent = applicantsSnapshot.size;
            ...
            document.getElementById('pendingInterviews').textContent = interviewsSnapshot.size;
            ...
            const passRate = (passedSnapshot.size / applicantsSnapshot.size * 100).toFixed(1);
    ...
    async initializeCharts() {
        ...
        const applicantProgressCtx = document.getElementById('applicantProgressChart').getContext('2d');
        ...
        try {
            ...
            snapshot.forEach(doc => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="action-btn btn-view" data-id="${doc.id}">View</button>
                    </td>
                `;
        ...
        try {
            ...
            snapshot.forEach(doc => {
                ...
                tr.innerHTML = `
                    ...
                    <td>
                        <button class="action-btn btn-view" data-id="${doc.id}">View</button>
                        <button class="action-btn btn-edit" data-id="${doc.id}">Edit</button>
                    </td>
                `;
                tbody.appendChild(tr);
            });
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            if (e.target.classList.contains('action-btn')) {
                ...
                const action = e.target.classList.contains('btn-view') ? 'view' : 'edit';
...
function updateStatistics() {
    ...
    document.querySelector('.stat-card.rejected .stat-number').textContent = rejected;
}
...
function renderApplicants() {
    ...
    paginatedApplicants.forEach(applicant => {
        ...
        row.innerHTML = `
            ...
            <td>
                <button class="btn-view" onclick="viewApplicant('${applicant.id}')">
...
function filterApplicants() {
    ...
    return applicants.filter(applicant => {
        ...
        const matchesProgram = !programFilter || applicant.program === programFilter;
...
function viewApplicant(applicantId) {
    ...
    document.getElementById('applicantName').textContent = `${applicant.firstName} ${applicant.lastName}`;
    document.getElementById('applicantId').textContent = `ID: ${applicant.id}`;
    document.getElementById('applicantStatus').className = `status-badge status-${applicant.status}`;
    document.getElementById('applicantStatus').textContent = applicant.status;

    // Personal Information
    document.getElementById('fullName').textContent = `${applicant.firstName} ${applicant.lastName}`;
    document.getElementById('email').textContent = applicant.email;
    document.getElementById('phone').textContent = applicant.phone;
    document.getElementById('dob').textContent = new Date(applicant.dateOfBirth).toLocaleDateString();
    ...
    if (applicant.examScore) {
        ...
        document.querySelector('.score-circle circle.progress').style.setProperty(
            '--progress',
        ...
        sections.forEach(section => {
            ...
            const progressBar = document.querySelector(`.score-bar:contains('${section}') .progress`);
            const scoreValue = document.querySelector(`.score-bar:contains('${section}') .score-value`);
...
async function updateStatus(status) {
    const applicantId = document.getElementById('applicantId').textContent.replace('ID: ', '');
    try {
        await firebase.firestore().collection('applicants').doc(applicantId).update({
            status: status,
...
function exportApplicants() {
    ...
    link.setAttribute('download', `applicants_export_${new Date().toISOString()}.csv`);
