...
export class ExamCreator {
  ...
  initializeEventListeners() {
    ...
    // Preview exam button
    document
      .querySelector(".preview-exam-btn")
  ...
  handleQuestionActions(target) {
    ...
    if (actionButton) {
      const actionClass = Array.from(actionButton.classList).find((cls) =>
        cls.endsWith("-btn")
      );
  ...
  addQuestion(type) {
    ...
    questionItem
      ...
      .forEach((element) => {
        element.id = `${
          element.id || element.getAttribute("name") || "field"
        }-${timestamp}`;
      });
  ...
  addAnswer(target) {
    const answersList = target
      .closest(".question-item")
      .querySelector(".answers-list");
    const timestamp = Date.now();
  ...
  addTestCase(target) {
    const testCasesList = target
      .closest(".question-item")
      .querySelector(".test-cases-list");
    const timestamp = Date.now();
  ...
  collectQuestionData(questionItem) {
    ...
    const baseData = {
      ...
      points: parseInt(
        questionItem.querySelector(".question-points")?.value || "1"
      ),
    };
